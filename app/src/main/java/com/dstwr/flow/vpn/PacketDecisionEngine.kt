...
/** Combines packet classification, identity, blocking, shaping and metering. */
class PacketDecisionEngine(
    ...
    suspend fun inspect(buffer: ByteArray, length: Int, direction: TrafficDirection): Decision {
        val packet = PacketParser.parse(buffer, length) ?: return Decision.Malformed
        connections.touch(packet)
        if (policies.isGlobalBlocked()) {
            meter.record(packet, dropped = true, throttled = false)
            return Decision.Blocked(packet, "*", direction)
        }
    ...
    suspend fun inspect(buffer: ByteArray, length: Int, packageName: String?, upload: Boolean): Decision {
        ...
        if (policies.isGlobalBlocked() || packageName?.let(policies::get)?.blocked == true) {
            meter.record(packet, dropped = true, throttled = false)
            return Decision.Blocked(packet, packageName ?: "*", direction)
        }
        ...
        val throttled = !result.allowed
        meter.record(packet, dropped = false, throttled = throttled)
        return if (throttled) Decision.Throttled(result.retryAfterMillis, packet, packageName, direction)
        else Decision.Forward(packet, packageName, direction)
    }

    sealed interface Decision {
        data object Malformed : Decision
        data class Blocked(val packet: ParsedPacket, val packageName: String, val direction: TrafficDirection) : Decision
        data class Forward(val packet: ParsedPacket, val packageName: String?, val direction: TrafficDirection) : Decision
        data class Throttled(val retryAfterMillis: Long, val packet: ParsedPacket, val packageName: String?, val direction: TrafficDirection) : Decision
    }
}
