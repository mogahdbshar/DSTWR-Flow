...
interface AppPolicyDao {
    ...
    suspend fun getAll(): List<AppPolicyEntity>

    @Query("SELECT * FROM app_policies WHERE packageName = :packageName LIMIT 1")
    suspend fun get(packageName: String): AppPolicyEntity?

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun upsert(policy: AppPolicyEntity)
...
interface UsageSnapshotDao {
    @Query("SELECT * FROM usage_snapshots WHERE startTime >= :since ORDER BY startTime DESC")
    suspend fun since(since: Long): List<UsageSnapshotEntity>

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insert(snapshot: UsageSnapshotEntity)

    @Query("DELETE FROM usage_snapshots WHERE startTime < :before")
